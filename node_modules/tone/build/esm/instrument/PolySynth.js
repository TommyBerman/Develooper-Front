...
var PolySynth = /** @class */ (function (_super) {
    ...
    function PolySynth() {
        ...
        _this.assert(!isNumber(options.voice), "DEPRECATED: The polyphony count is no longer the first argument.");
     ...
     */
    PolySynth.prototype._getActiveVoice = function (note) {
        var event = this._activeVoices.find(function (_a) {
            var midi = _a.midi;
            return midi === note;
        });
        if (event) {
    ...
    PolySynth.prototype._triggerAttack = function (notes, time, velocity) {
        ...
        notes.forEach(function (note) {
            ...
            var voice = _this._getActiveVoice(midiNote);
            // if it has a note, and that note is still active
            if (voice && voice.getLevelAtTime(time) > 0) {
                var activeVoiceIndex = _this._activeVoices.findIndex(function (e) { return e.voice === voice; });
                _this._activeVoices.splice(activeVoiceIndex, 1);
            }
            else {
                // otherwise get the next available voice
                voice = _this._getNextAvailableVoice();
            }
            if (voice) {
                voice.triggerAttack(note, time, velocity);
                _this._activeVoices.unshift({
                    midi: midiNote, voice: voice,
                });
    ...
    PolySynth.prototype.set = function (options) {
        ...
        this._voices.forEach(function (voice) { return voice.set(_this.options); });
        return this;
    };
    /**
